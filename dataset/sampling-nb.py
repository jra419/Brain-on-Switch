...
def main():
    ...
    random.shuffle(test)
    with open('./{}/json-nb/train.json'.format(args.dataset), 'w') as fp:
        json.dump(train, fp, indent=1)
    with open('./{}/json-nb/test.json'.format(args.dataset), 'w') as fp:
    ...
    with open('./{}/json-nb/statistics.json'.format(args.dataset), 'w') as fp:
        json.dump({
